...
class Arpeggiator(threading.Thread):
    ...
    Basic sequencer
    """
    def __init__(self, time_keeper, beats, note_length, root, tone, chord):
        ...
        if tone == 'maj':
            self.steps = audio.major(root, chord, 0.05, arp=True)
        elif tone == 'min':
            self.steps = audio.minor(root, chord, 0.05, arp=True)
        elif tone == 'dom':
            self.steps = audio.dominant(root, chord, 0.05, arp=True)
        self.running = False
